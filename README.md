...
Every candidate runs the automated baseline and the deployed smoke journey as release stages above. The **public launch** additionally runs the complete fail-closed [Security Release Gate](docs/security-verification.md) — the protected `Security Gate` workflow, dispatched against the deployed candidate before it is made public, and again after any Security-Relevant Change. It runs the automated suites, records what each proved as the verification catalog's stable check identifiers, reads back the manual abuse cases, live configuration assertions and CSP journeys the Project Owner recorded through the `Security Evidence` workflow, reads the two recovery drills from the readiness exercises they wrote themselves, and assembles one dated Security Gate Record for the exact candidate.

The gate runs against a deployment rather than before one, because the checks it exists for — the deployed role and cross-role smokes, the live browser policy, the served-source-map probe, the live provider configuration — have nothing to observe until the candidate is running. That is the report-only-then-enforced rollout the threat model describes: the candidate is deployed privately, the gate is what decides whether it becomes public.
