...
Use Sentry email for application, worker, reconciliation, third-party-integration, Sentry-pattern, and abuse incidents; GitHub Actions notifications for deployment, Canonical Data Rebuild, backup verification, and recovery-drill failures; and Railway billing email for its native warning and hard limit. The repository's **Owner Diagnostics** workflow prints the current-state summary and is safe to dispatch at any time, including during maintenance; it is evidence, not an alert channel. Reading changes nothing. It is also where a clearing rule that ends in a verification only a person can perform — an authorization smoke, an integration smoke, a rotated credential — is recorded against the incident's correlation identifier, and where the daily cost projection is made from the figures Railway shows. There is no pager, SMS, public issue, User notification, external uptime monitor, or public status page. If an originating service cannot emit recovery, send a privacy-safe recovery event through Sentry.
...
The required dated evidence record is defined in [operational readiness evidence](readiness-evidence.md) and is produced by the repository's **Readiness Evidence** workflow, which reads back the exercises recorded against the exact candidate rather than accepting typed-in results. It reads and changes nothing, so it is safe to dispatch during maintenance. Security-specific proof additionally follows the [Security Release Gate](../security-verification.md), produced by the repository's **Security Gate** workflow; the Project Owner records each manual abuse case, live configuration assertion, and CSP rollout journey through the **Security Evidence** workflow as they perform it. Every readiness row except backups and recovery verification is written by that gate from the checks it ran, so a candidate whose gate is blocked has failed readiness rows rather than unexercised ones.
