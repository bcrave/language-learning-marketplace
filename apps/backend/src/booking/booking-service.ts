...
async function recordBookingAudit(
  db: Database,
  studentId: string,
  operation: "booking.created" | "booking.cancelled",
  targetId: string,
...
async function notifyStudent(
  ...
  messageId: "booking.created.student" | "booking.cancelled.student",
...
export async function bookClassSession(
  ...
  const booking = await transaction.insertInto("bookings").values({
    ...
    ended_at: null,
  }).returning(["id", "booked_at"]).executeTakeFirstOrThrow();
...
export async function cancelBooking(
  ...
  await transaction.updateTable("schedule_commitments").set({ active: false })
    .where("class_session_id", "=", session.id).where("user_id", "=", student.id)
  ...
  await transaction.updateTable("class_session_reminders")
    ...
    .where("class_session_id", "=", session.id).where("recipient_user_id", "=", student.id)
