      ...
      <div>
        ...
        {courses.map((course) => <article className="course-card" key={course.id}>
          ...
          <ol>{course.lessonUnits.map((unit) => <li key={unit.id}>
            ...
            {unit.materials.map((material) => <details key={material.id}><summary>{material.title} · {material.kind}</summary>{material.structuredContent && <pre>{material.structuredContent}</pre>}{material.httpsUrl && <p><a href={material.httpsUrl}>{material.publisher ?? material.httpsUrl}</a></p>}</details>)}
